...
func TestFindSessionTranscript(t *testing.T) {
	...
	// We can't test findSessionTranscript directly without mocking the home directory,
...
func TestGetSessionTokens_RealProject(t *testing.T) {
	...
	t.Logf("  Input:  %s (%d)", formatTokenCount(tokens.Input), tokens.Input)
	t.Logf("  Output: %s (%d)", formatTokenCount(tokens.Output), tokens.Output)
	t.Logf("  Total:  %s (%d)", formatTokenCount(tokens.Total), tokens.Total)
