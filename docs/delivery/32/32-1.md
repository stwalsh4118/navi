...
| 2026-02-09 18:00:28 | Status Change | Proposed | InProgress | Started implementation | AI_Agent |
| 2026-02-09 18:10:00 | Status Change | InProgress | Review | Implementation complete, ready for review | AI_Agent |
...
- `internal/tui/model.go` - Added `taskScrollOffset` field, `taskPanelMaxScroll()`, `ensureTaskCursorVisible()`, `clampTaskScrollOffset()`, `taskPanelViewportLines()`, PgUp/PgDn/g/G keybindings, scroll offset reset on project change
- `internal/tui/taskview.go` - Modified `renderTaskPanelList()` to use offset-based viewport slicing
- `internal/tui/preview.go` - Added `taskPanelPageScrollAmt` constant
- `internal/tui/task_scroll_test.go` - New test file with comprehensive scroll tests
