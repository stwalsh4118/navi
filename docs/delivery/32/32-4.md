...
| 2026-02-09 18:30:00 | Status Change | Proposed | InProgress | Started implementation | AI_Agent |
| 2026-02-09 19:00:00 | Status Change | InProgress | Review | Implementation complete, ready for review | AI_Agent |
...
- [x] Task panel shows scroll indicators when content overflows
- [x] Session list shows scroll indicators when sessions overflow
- [x] Preview pane shows scroll indicators when content overflows
- [x] No indicators shown when all content is visible
- [x] Indicators update dynamically as user scrolls
- [x] Existing tests pass
- [x] New tests cover indicator visibility logic

## Files Modified

- `internal/tui/preview.go` - Added `scrollIndicatorAbove` and `scrollIndicatorBelow` format string constants
- `internal/tui/taskview.go` - Modified `renderTaskPanelList()` to show dimmed scroll indicators above/below viewport, reducing content lines to maintain total height
- `internal/tui/view.go` - Modified `renderSessionList()` to show top/bottom indicators based on scroll offset and maxVisible; modified `renderPreview()` to show indicators based on scroll position
- `internal/tui/scroll_indicator_test.go` - New test file with 18 test cases covering all three panels: indicator visibility, correct counts, no indicators when content fits, dynamic updates, empty content, and dim styling
