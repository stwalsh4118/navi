...
| 2026-02-12 07:56:45 | Status Change | Proposed | InProgress | Started implementation | AI_Agent |
| 2026-02-12 07:58:38 | Status Change | InProgress | Review | Implementation complete, ready for review | AI_Agent |
...
- [x] `J`/`K` jump between group headers correctly
- [x] `e` toggles expand/collapse all
- [x] `a` toggles accordion mode
- [x] Accordion mode collapses other groups on expand
- [x] Header shows `accordion` indicator when active
...
- `internal/tui/model.go` — Added `J`, `K`, `e`, `a` keybindings in `updateTaskPanelFocus()`; added `jumpToNextGroup()`, `jumpToPrevGroup()`, `toggleExpandCollapseAll()`, `applyAccordion()`, `toggleGroupExpansion()` methods; refactored space/enter group toggle to use shared method with accordion support
- `internal/tui/taskview_nav_test.go` — New test file for group jumping, expand/collapse all, and accordion mode
