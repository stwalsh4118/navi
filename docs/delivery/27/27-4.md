...
| 2026-02-06 17:38:38 | Status Change | Review | Done | Task completed and verified; also added automatic remote git polling | AI_Agent |
...
- `internal/tui/model.go` - Added remoteGitInfoMsg type, modified G key handler, added remoteGitInfoMsg handler, updated git cache merging to include remote sessions, added pollAllGitInfoCmd() for batched local+remote polling
- `internal/tui/sessions.go` - Added fetchRemoteGitCmd(), pollRemoteGitInfoCmd(), added remote import; filtered pollGitInfoCmd() to local sessions only
