...
| 2026-02-12 09:29:31 | Status Change | Proposed | InProgress | Started implementation (parallel with 38-5) | AI_Agent |
| 2026-02-12 09:36:32 | Status Change | InProgress | Review | Implementation complete, ready for review | AI_Agent |
...
- `internal/git/pr.go` — Added `GetPRDetail`, `GetPRDetailByRepo`, `parsePRDetailJSON`, `parseStatusCheck`, `mapStatusContextState`, `computeCheckSummary`, `JoinNames` functions and intermediate JSON parsing types
- `internal/git/pr_test.go` — Added 10 test functions for PR detail parsing, status checks, reviews, error cases
- `internal/tui/model.go` — Extended `gitPRMsg` with `prDetail` field; updated handler to store PRDetail
- `internal/tui/sessions.go` — Updated `fetchPRCmd` and `fetchRemotePRCmd` to use `GetPRDetail`/`GetPRDetailByRepo`
