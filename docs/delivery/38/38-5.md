...
| 2026-02-12 09:29:31 | Status Change | Proposed | InProgress | Started implementation (parallel with 38-2) | AI_Agent |
...
- `internal/git/pr.go` — Added `GetPRComments`, `GetPRCommentsByRepo`, `parseReviewComments`, `parseGeneralComments`, `sortCommentsByCreatedAt` functions and API JSON types
- `internal/git/pr_test.go` — Added 9 test functions for comment parsing, sorting, JSON serialization, validation
- `internal/tui/model.go` — Added `gitPRCommentsMsg` message type
- `internal/tui/sessions.go` — Added `fetchPRCommentsCmd` and `fetchRemotePRCommentsCmd`
