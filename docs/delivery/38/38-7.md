...
| 2026-02-12 09:40:46 | Status Change | Proposed | InProgress | Started implementation | AI_Agent |
| 2026-02-12 09:44:58 | Status Change | InProgress | Review | Implementation complete, ready for review | AI_Agent |
...
- `internal/tui/model.go` — Added `prAutoRefreshTickMsg`, `prAutoRefreshInterval`, `prAutoRefreshActive` field, tick handler, updated `gitPRMsg`/`esc`/`r` handlers
- `internal/tui/sessions.go` — Added `prAutoRefreshTickCmd()`
- `internal/tui/view.go` — Added "Auto-refreshing..." indicator in git detail view
