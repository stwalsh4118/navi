...
| 2026-02-12 09:36:32 | Status Change | Proposed | InProgress | Started implementation (parallel with 38-3) | AI_Agent |
...
- Show aggregate check indicator after PR number:
  ...
  - `[PR#42 ✗]` — Any check failed
  - `[PR#42 ●]` — Checks pending or in progress
  - `[PR#42]` — No checks configured (current behavior)
- Show comment count when > 0: `[PR#42 ✓ 💬5]`
...
- Test indicator with all checks passing: `[PR#42 ✓]`
- Test indicator with failing checks: `[PR#42 ✗]`
- Test indicator with pending checks: `[PR#42 ●]`
- Test indicator with no checks: `[PR#42]`
- Test indicator with comments: `[PR#42 ✓ 💬5]`
...
- `internal/tui/view.go` — Enhanced `renderGitInfo()` PR indicator to show check status, comment count, and draft state
