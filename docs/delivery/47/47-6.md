...
- Added `internal/tui/pmview_test.go` integration-style PM view test coverage across toggle lifecycle, projects rendering/sorting, project Enter selection + filtering, events ordering/scrolling, focus switching, expansion, and responsive behavior.
- Verified PM-specific race run and full-suite stability.
- Verification commands:
  - `go test -race ./internal/tui/... -run TestPM`
  - `go test ./internal/tui/... -run TestPM`
  - `go build ./...`
  - `go test ./...`

- Internal AI review gate: `PASS` (no outstanding critical/high findings).
