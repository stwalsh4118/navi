...
| 2026-02-19 08:53:24 | Status Change | Proposed | InProgress | Started implementation | AI_Agent |
| 2026-02-19 08:55:28 | Status Change | InProgress | Review | Implementation complete, internal AI review passed | AI_Agent |
| 2026-02-20 05:48:29 | Status Change | Review | Done | Task completed and verified | AI_Agent |

## Requirements

1. Add a RAM badge to `renderMetricsBadges()` using the format `"🧠 256M"` (brain emoji + formatted bytes)
...
5. Follow existing dimmed style — no new styles needed
...
That's the entire change — one conditional block, following the exact same pattern as the three existing badges.
...
- [ ] RAM badge renders with format `"🧠 <value>"` on the metrics line
...
- `internal/tui/view.go` — extend `renderMetricsBadges()` with RAM badge
