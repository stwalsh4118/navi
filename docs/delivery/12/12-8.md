...
| 2026-02-05 16:41:49 | Status Change | Proposed | InProgress | Started implementation | AI_Agent |
| 2026-02-05 16:43:01 | Status Change | InProgress | Review | Implementation complete, ready for review | AI_Agent |
...
- Token section NOT displayed - token tracking is not available (see task 12-4)
- Time section shows: started time, duration, working/waiting breakdown with percentages
- Tool section shows: total calls, tool counts sorted by frequency, recent tools list
- Dialog uses consistent styling with other detail views

## Files Modified

- `dialog.go` - Added DialogMetricsDetail mode and title
- `model.go` - Added 'i' keybinding to open metrics dialog
- `view.go` - Added renderMetricsDetailView function and dialog case
