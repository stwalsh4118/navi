...
| 2026-02-19 07:21:10 | Status Change | Proposed | InProgress | Started implementation | AI_Agent |
| 2026-02-19 07:22:19 | Status Change | InProgress | Review | Implementation complete, internal AI review passed | AI_Agent |
...
**Files to modify:**
- `internal/tui/model.go` — add `updateSoundPackPicker(msg tea.KeyMsg)` and wire into `updateDialog`
...
- `internal/tui/model.go` — added `updateSoundPackPicker()` method with navigation, selection, preview, cancel; `previewSoundPackCmd()`; routed in `updateDialog()`
