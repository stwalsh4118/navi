...
| 2026-02-19 07:12:08 | Status Change | Proposed | InProgress | Started implementation | AI_Agent |
| 2026-02-19 07:19:25 | Status Change | InProgress | Review | Implementation complete, internal AI review passed | AI_Agent |
...
- `internal/audio/notifier.go` — added `SetPack()`, `ActivePack()` methods; upgraded mutex to `sync.RWMutex`; added RLock to `resolveSound()`
- `internal/audio/config.go` — added `SavePackSelection()` function with file permission preservation
- `internal/audio/notifier_test.go` — added tests for SetPack (valid, empty, invalid, nil, concurrent), ActivePack
- `internal/audio/config_test.go` — added tests for SavePackSelection (new file, preserves settings, clears, creates parent dir)
