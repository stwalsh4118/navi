...
| 2026-02-06 18:19:46 | Status Change | Proposed | InProgress | Started implementation | AI_Agent |
| 2026-02-06 18:22:00 | Status Change | InProgress | Review | Implementation complete | AI_Agent |
...
- `internal/tui/filter.go` (new) — SortMode type, constants, labels, status filter keys, sort/filter/fuzzy functions
- `internal/tui/model.go` — Extended Model struct with search/filter/sort fields; updated InitialModel defaults
- `internal/tui/input.go` — Added initSearchInput() and inputSearchCharLimit constant
