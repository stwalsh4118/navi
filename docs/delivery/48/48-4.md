...
| 2026-02-18 09:12:44 | Status Change | Proposed | InProgress | Started implementation | AI_Agent |
| 2026-02-18 09:17:21 | Status Change | InProgress | Review | Implementation complete, internal AI review passed | AI_Agent |
| 2026-02-18 13:05:37 | Status Change | Review | Done | Task completed and verified. Evolved to fresh-session-per-invocation design (no --resume). Added streaming, model selection (sonnet), candidate JSON fallback, usage tracking. | AI_Agent |
...
4. CLI flag assembly:
   ...
   - `--json-schema <schema-json>` (schema file contents loaded and passed as JSON string)
...
- Researched Claude CLI docs and documented usage in `docs/delivery/48/48-4-claude-cli-guide.md`.
- `go test ./internal/pm` passed.
- Verified first-run invocation captures and persists session ID.
- Verified resume invocation includes `--resume <session-id>`.
- Verified CLI arg assembly includes `-p`, `--output-format json`, `--json-schema`, `--allowedTools`, and `--system-prompt-file`.
- Verified inbox JSON is piped to stdin in helper-process tests.
- Verified timeout path returns wrapped invocation errors.
- Internal AI review gate verdict: `pass` (no critical/high findings).

## Files Modified

- `internal/pm/invoker.go`
- `internal/pm/invoker_test.go`
- `docs/delivery/48/48-4-claude-cli-guide.md`
