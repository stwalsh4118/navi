...
func TestEscapeClosesDialog(t *testing.T) {
	testCases := []struct {
		...
		{"closes new session dialog", DialogNewSession},
...
func TestNewSessionKeyOpensDialog(t *testing.T) {
	t.Run("n opens new session dialog", func(t *testing.T) {
		m := Model{
			width:      80,
			height:     24,
			dialogMode: DialogNone,
	...
	t.Run("n does nothing when new session dialog open", func(t *testing.T) {
		m := Model{
			width:      80,
			height:     24,
		...
		// Should still be new session dialog (n just types into input)
		if updated.dialogMode != DialogNewSession {
...
func TestKillSessionKeyOpensDialog(t *testing.T) {
	t.Run("x opens kill confirmation dialog", func(t *testing.T) {
		session := SessionInfo{TmuxSession: "test-session"}
		m := Model{
			width:      80,
			height:     24,
			dialogMode: DialogNone,
...
func TestKillConfirmDialogRender(t *testing.T) {
	...
	if !strings.Contains(result, "my-session") {
		t.Error("dialog should contain session name")
	}
...
func TestRenameSessionKeyOpensDialog(t *testing.T) {
	...
	t.Run("R pre-fills name input with current session name", func(t *testing.T) {
		session := SessionInfo{TmuxSession: "my-session"}
		m := Model{
			width:      80,
			height:     24,
...
func TestRenameDialogRender(t *testing.T) {
	session := SessionInfo{TmuxSession: "old-name"}
	m := Model{
		width:           80,
		height:          24,
	...
	if !strings.Contains(result, "old-name") {
		t.Error("dialog should show current session name")
	}
...
func TestRenameSessionEnterSameName(t *testing.T) {
	session := SessionInfo{TmuxSession: "same-name"}
	m := Model{
		width:           80,
		height:          24,
		dialogMode:      DialogRename,
