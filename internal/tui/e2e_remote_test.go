...
`
		...
		err = remote.ValidateConfig(&config.Remotes[0])
		if err == nil {
...
`
		...
		remote.ApplyDefaults(&config.Remotes[0])

		if config.Remotes[0].SessionsDir != remote.DefaultSessionsDir {
			t.Errorf("expected default sessions_dir %q, got %q", remote.DefaultSessionsDir, config.Remotes[0].SessionsDir)
		}
	})
}
...
func TestE2E_SessionDisplay(t *testing.T) {
	t.Run("remote sessions appear in s list", func(t *testing.T) {
		m := Model{
			width:  120,
			height: 24,
	...
	t.Run("local and remote sessions sort correctly", func(t *testing.T) {
		...
		if sessions[0].TmuxSession != "new-remote" {
			t.Errorf("most recent s should be first, got %s", sessions[0].TmuxSession)
		}
	})
}
...
func TestE2E_RemoteAttach(t *testing.T) {
	t.Run("SSH command constructed correctly", func(t *testing.T) {
		...
		args := remote.BuildSSHAttachCommand(r, "my-session")
		...
		if !strings.Contains(commandStr, "my-session") {
			t.Error("SSH command should include s name")
		}
	})
	...
	t.Run("jump host included when configured (CoS 8)", func(t *testing.T) {
		...
		args := remote.BuildSSHAttachCommand(r, "test-session")
	...
	t.Run("enter key triggers remote attach for remote session", func(t *testing.T) {
		...
		// Should store the s name for cursor restoration
...
func TestE2E_ConnectionPooling(t *testing.T) {
	t.Run("remote.SSHPool manages connections by remote name", func(t *testing.T) {
		remotes := []remote.Config{
			{Name: "dev", Host: "dev.example.com", User: "user", Key: "~/.ssh/key"},
			{Name: "staging", Host: "staging.example.com", User: "admin", Key: "~/.ssh/key"},
		}

		pool := remote.NewSSHPool(remotes)
		if pool == nil {
			t.Fatal("remote.NewSSHPool should return non-nil pool")
		}

		// Verify remotes are tracked
		if len(pool.RemoteNames()) != 2 {
			t.Errorf("pool should have 2 remotes, got %d", len(pool.RemoteNames()))
		}

		// Verify initial status is disconnected
		status := pool.GetStatus("dev")
		if status.Status != remote.StatusDisconnected {
...
func TestE2E_ErrorHandling(t *testing.T) {
	...
	t.Run("remote sessions merged without crash when poll fails", func(t *testing.T) {
		...
		msg := remoteSessionsMsg{sessions: []session.Info{}}
...
func TestE2E_Filtering(t *testing.T) {
	...
	t.Run("filtered list shows correct sessions", func(t *testing.T) {
		...
		if len(filtered) != 2 {
			t.Errorf("session.FilterLocal should return 2 sessions, got %d", len(filtered))
		}

		for _, s := range filtered {
			if s.Remote != "" {
				t.Errorf("session.FilterLocal should only return local sessions, got remote: %s", s.Remote)
			}
		}
	})
...
// TestE2E_JSONParsing tests remote s JSON parsing
...
{"tmux_session":"session2","status":"idle","cwd":"/home/user/project2","timestamp":1234567891}`
	...
	t.Run("handles empty output", func(t *testing.T) {
		sessions := remote.ParseSessionOutput("", "dev")
	...
	t.Run("handles single JSON object", func(t *testing.T) {
		...
		sessions := remote.ParseSessionOutput(output, "dev")

		if len(sessions) != 1 {
			t.Errorf("expected 1 s, got %d", len(sessions))
		}
	})
}

// Helper function to parse YAML (for testing)
func parseRemotesYAML(data []byte) (*remote.RemotesConfig, error) {
	config, err := remote.ParseConfigData(data)
	if err != nil {
		return nil, err
	}
	return &config, nil
