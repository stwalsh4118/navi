...
// E2E tests for PBI-12: Session metrics.Metrics
...
func TestE2E_MetricsBadgesDisplayed(t *testing.T) {
	t.Run("session with metrics shows badges in row", func(t *testing.T) {
		...
		if !strings.Contains(result, "⏱") {
			t.Error("CoS 2 failed: Time badge should be displayed in s row")
		}
...
// metrics view showing comprehensive s metrics.
...
func TestE2E_AggregateDashboard(t *testing.T) {
		...
		m.sessions = []session.Info{
			{
				...
				Metrics: &metrics.Metrics{
					...
					Tools: &metrics.ToolMetrics{Counts: map[string]int{"Read": 20}},
				},
			},
			{
				...
				Status:      "waiting",
				Metrics: &metrics.Metrics{
					Time:  &metrics.TimeMetrics{TotalSeconds: 1800},
					Tools: &metrics.ToolMetrics{Counts: map[string]int{"Edit": 10}},
				},
			},
			{
				...
				Status:      "working",
				Metrics: &metrics.Metrics{
					Time:  &metrics.TimeMetrics{TotalSeconds: 7200},
					Tools: &metrics.ToolMetrics{Counts: map[string]int{"Bash": 15}},
				},
			},
		}
	...
	t.Run("aggregate calculation sums correctly", func(t *testing.T) {
		...
		aggregate := session.AggregateMetrics(sessions)
		...
		totalTools := metrics.FormatToolCount(aggregate.Tools)
		if totalTools != 26 {
...
// TestE2E_MetricsRealTimeUpdates verifies that metrics update with s changes.
func TestE2E_MetricsRealTimeUpdates(t *testing.T) {
	t.Run("metrics update when sessions change", func(t *testing.T) {
		...
		m.sessions = []session.Info{
			{
				...
				Metrics: &metrics.Metrics{
					Time:  &metrics.TimeMetrics{TotalSeconds: 3600},
					Tools: &metrics.ToolMetrics{Counts: map[string]int{"Read": 10}},
				},
			},
		}
		...
		// Add another s (simulating poll update)
		m.sessions = append(m.sessions, session.Info{
			TmuxSession: "session2",
			Metrics: &metrics.Metrics{
				Time:  &metrics.TimeMetrics{TotalSeconds: 1800},
				Tools: &metrics.ToolMetrics{Counts: map[string]int{"Edit": 5}},
			},
		})
...
// TestE2E_MetricsPersistence verifies that metrics persist in s files.
func TestE2E_MetricsPersistence(t *testing.T) {
	t.Run("session info with metrics serializes correctly", func(t *testing.T) {
		...
		if s.Metrics == nil {
			t.Error("CoS 7 failed: metrics.Metrics should be present")
		}
