...
func TestConnectionStatusString(t *testing.T) {
	tests := []struct {
		...
		{remote.StatusConnected, "connected"},
		{remote.StatusDisconnected, "disconnected"},
		{remote.StatusError, "error"},
		{remote.ConnectionStatus(99), "unknown"},
	}

	for _, tt := range tests {
		got := tt.status.String()
		if got != tt.want {
			t.Errorf("remote.ConnectionStatus(%d).String() = %v, want %v", tt.status, got, tt.want)
		}
	}
}
...
func TestNewSSHPool(t *testing.T) {
	...
	t.Run("initializes remotes map", func(t *testing.T) {
		if len(pool.RemoteNames()) != 2 {
			t.Errorf("remotes count = %d, want 2", len(pool.RemoteNames()))
		}
	})

	t.Run("initializes status map", func(t *testing.T) {
		if len(pool.GetAllStatus()) != 2 {
			t.Errorf("status count = %d, want 2", len(pool.GetAllStatus()))
		}
	})

	t.Run("all remotes start disconnected", func(t *testing.T) {
		for _, name := range pool.RemoteNames() {
...
func TestRemoteStatus(t *testing.T) {
	status := &remote.RemoteStatus{
		Status:    remote.StatusConnected,
		LastError: nil,
		LastPoll:  time.Now(),
	}

	if status.Status != remote.StatusConnected {
