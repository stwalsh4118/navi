...
> Rationale: Defines the standard tools used in this project to ensure consistency.

- **Package Manager**: This project uses **pnpm** (not npm or yarn). Always use `pnpm` commands:
...
- Consult API specifications before implementing to avoid duplication.
...
- **Required Sections**:
  ...
  - `## Implementation Plan`
  - `## Verification`
  - `## Files Modified` (must include list of API specification files updated, if any)
...
- **event_transition on "start_work" from Agreed to InProgress**:
  ...
  1a. Identify which systems/services the task will modify.
  1b. Load the relevant API specification files from `docs/api-specs/` into context before beginning implementation.
...
- **event_transition on "submit_for_review" from InProgress to Review**:
  ...
  4. Run automated code review using `coderabbit review --base main --plain` to perform AI-driven code review.
  5. Review the coderabbit results and address any critical issues found before proceeding.
  6. Update task documentation with implementation details.
  6a. If the task created or modified any APIs, update the relevant API specification files in `docs/api-specs/`.
  6b. Document which API specs were updated in the task's 'Files Modified' section.
  7. Create a pull request or mark as ready for review.
  8. Notify the User that review is needed, including a summary of linter and coderabbit review results.
  9. Log submission for review in task history.
...
### 4.6.1 Code Quality Tools

> Rationale: Ensures consistent code quality and catches common issues before code review.

Before submitting any code for review, the following tools MUST be run and all issues addressed:
...
cd api && make check
```
This runs all quality checks: formatting, vet, lint, and tests.

**Individual commands:**

1. **golangci-lint**: Comprehensive linting with multiple analyzers
   ```bash
   cd api && make lint
   # or: golangci-lint run
   ```
   - Configuration: `api/.golangci.yml`
   - Must pass with zero errors
   - Warnings should be addressed or justified

2. **go vet**: Official Go static analysis tool
   ```bash
   cd api && make vet
   # or: go vet ./...
   ```
...
3. **go test**: Run all tests with race detector
   ```bash
   cd api && make test-race
   # or: go test -race ./...
   ```
   - All tests must pass
...
4. **Test coverage**: Verify adequate test coverage
   ...
   - Report coverage in task documentation

5. **Format code**: Auto-fix formatting issues
   ```bash
   cd api && make fmt
   # or: gofmt -w .
   ```
...
For future web development, establish similar tools:
- ESLint for linting
...
- If a linter rule must be disabled for a valid reason, add an inline comment explaining why
- Document any disabled rules in the task's review section
- Never disable security-related checks without explicit approval
...
1. **Commit Message Format**:
   - All commits MUST use conventional commit style: `<type>(<scope>): <subject>`
   - Common types: `feat`, `fix`, `docs`, `chore`, `test`, `refactor`, `perf`
   - Scope should indicate the component/package being modified (e.g., `cursor`, `config`, `deps`)
   - Subject should be concise and descriptive
   - Optional body can include detailed changes with bullet points
   - When a task moves from `Review` to `Done`, commits should reference the task ID in the body or footer
   - Examples:
     - `feat(cursor): implement file system watcher for database file`
     - `chore(deps): add fsnotify dependency for file system watching`
     - `docs(cursor): update API spec and mark task 2-3 as done`
     - `fix(config): handle empty cursor log path validation`

2. **Pull Request**:
   - Title: `[<task_id>] <conventional commit subject>`
   - Include a link to the task in the description
   - Ensure all task requirements are met before marking as `Done`

3. **Automation**:
   - When a task is marked as `Done`, create commits using conventional commit format
   - Group related changes into logical commits (e.g., dependencies, implementation, tests, documentation)
   - Each commit should follow the format: `<type>(<scope>): <subject>`

4. **Verification**:
   - Ensure commits follow conventional commit format
   - Confirm the task status is updated in both the task file and index
   - Verify commit messages are descriptive and properly scoped
...
> Rationale: Maintains a centralized, concise reference of all system APIs to prevent code duplication and ensure consistency across implementation tasks.

## 6.1 Purpose and Principles

> Rationale: Establishes the role of API specifications in preventing duplicate implementations and maintaining system consistency.

- API specs serve as a concise reference to prevent code duplication
- Specs document existing APIs, interfaces, schemas, and data contracts
- Must be consulted before implementing new functionality
- Updated whenever APIs are created or modified

## 6.2 Location and Organization

> Rationale: Defines where and how API specifications are stored and organized for easy discovery and maintenance.

- **Location Pattern**: `docs/api-specs/<system>/`
- **Examples**: 
  - `docs/api-specs/media/`
  - `docs/api-specs/channels/`
  - `docs/api-specs/streaming/`
  - `docs/api-specs/database/`
- **File Naming**: One markdown file per logical grouping: `<system>-api.md`
  - Example: `docs/api-specs/media/media-api.md`

## 6.3 Required Content Structure

> Rationale: Standardizes API specification content to ensure all necessary information is captured consistently.

Each API spec file must include the following sections as applicable:

- **REST Endpoints**: HTTP method, path, request/response types (concise format)
- **Service Interfaces**: Public methods with function signatures
- **Database Schemas**: Table structures, columns, and relationships
- **Data Contracts**: Key types, interfaces, and data structures

## 6.4 Format Guidelines

> Rationale: Keeps specifications focused and actionable while avoiding unnecessary verbosity.

- Keep specs concise - just enough information to know what exists and how to use it
- Use code blocks for examples and signatures
- Include TypeScript/Go type definitions where applicable
- No verbose descriptions - focus on signatures, parameters, and brief usage examples
- Include only essential information needed to avoid reimplementation

## 6.5 Spec Update Requirements

> Rationale: Ensures API specifications remain current with the codebase through the task completion workflow.

- Update specs as part of task completion workflow (see Section 4.6 "submit_for_review")
- When a task creates or modifies any APIs:
  - Update the relevant API specification files in `docs/api-specs/`
  - Document which API specs were updated in the task's 'Files Modified' section
- If creating a new system/service, create the appropriate directory and API spec file
- Keep updates minimal and focused on what changed

## 6.6 Spec Loading Requirements

> Rationale: Ensures AI agents have context about existing APIs before implementing new functionality, preventing duplication, while optimizing context usage by loading only relevant specs.

- When starting work on a task (see Section 4.6 "start_work"):
  1. Read `docs/api-specs/README.md` to understand available specs and their organization
  2. Identify which specific spec files are relevant to the task based on:
     - Components being modified (load component-specific specs)
     - APIs being called (load API client specs)
     - State management involved (load state specs)
     - Styling changes (load styling/design token specs)
  3. Load ONLY the specific relevant spec files
  4. Do NOT load specs for unrelated domains
- Consult specs to determine if functionality already exists before creating new implementations
- If similar functionality exists, reuse or extend rather than recreate

**Examples**:
- Task modifying ProductGrid: Load `components/maps-components.md` only
- Task adding new API endpoint: Load `api-clients/woocommerce.md` + `data/types.md` + `api-clients/tanstack-query.md`
- Task updating styling: Load `core/styling.md` + `core/design-tokens.md` only
- Task modifying filter logic: Load `state/hooks.md` + `data/types.md` only
- Task creating new screen: Load relevant screen spec as reference + relevant component specs

## 6.7 Infrastructure Components and Reusable Code

> Rationale: Ensures infrastructure components and foundational code are documented and reused, not recreated.

- **Infrastructure Scope**: Internal packages that provide cross-cutting concerns (logging, configuration, middleware, database connections, etc.) are considered infrastructure
- **Documentation Requirement**: All infrastructure components MUST be documented in the appropriate spec files:
  - Core infrastructure: `docs/api-specs/core/` (project structure, styling, design tokens)
  - API clients: `docs/api-specs/api-clients/` (TanStack Query, WooCommerce, WordPress)
  - State management: `docs/api-specs/state/` (Zustand stores, custom hooks)
- **Reuse Mandate**: ALWAYS check relevant spec files in `docs/api-specs/` before implementing any infrastructure functionality. Use the README.md index to find the right spec files.
- **Examples of Infrastructure**:
  - Logging systems (e.g., zerolog logger package)
  - Configuration management
  - Database connection pooling
  - HTTP middleware (authentication, CORS, request logging)
  - Error handling utilities
  - Common validation functions
  - Styling systems (NativeWind, design tokens)
  - State management (Zustand stores)
  - API clients (TanStack Query hooks)
- **Rule**: If infrastructure exists, ALWAYS import and use it. NEVER recreate similar functionality
- **Updates**: When tasks create or modify infrastructure components, update the relevant spec files in `docs/api-specs/` (use README.md to identify which files to update)
```